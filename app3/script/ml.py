...
FEATURED_DIR = os.path.join('data', 'featured')

def save_raw_data(data, filename='keystrokes_raw'):
    ...
    print(f"Data saved raw data to {filepath}")

def save_featured_data(data, filename='keystrokes_featured'):
    if not os.path.exists(FEATURED_DIR):
        os.makedirs(FEATURED_DIR)
    
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    filepath = os.path.join(FEATURED_DIR, f'{filename}_{timestamp}.csv')
    data.to_csv(filepath, index=False)
    print(f"Data saved raw data to {filepath}")
def record_keystrokes():
    ...
    df = pd.read_csv('data/raw/keystroke_data.csv')
    hold_times = []
    flight_times = []
    ...
    save_featured_data(df_filtered)
...
if __name__ == "__main__":
    keystroke_df = record_keystrokes()
